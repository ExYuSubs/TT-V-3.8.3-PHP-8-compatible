...
//==========|  Funkcija za preuzimanje TTMDB podataka   |==========//

$TTIMDB_CACHE = array();

function tt_get_imdb_cache($torrentid)
{
    global $TTIMDB_CACHE;

    if (isset($TTIMDB_CACHE[$torrentid])) {
        return $TTIMDB_CACHE[$torrentid];
    }

    $file = $_SERVER['DOCUMENT_ROOT']."/cache/imdb/".$torrentid.".cache";

    if (!file_exists($file)) {
        $TTIMDB_CACHE[$torrentid] = false;
        return false;
    }

    $data = @unserialize(file_get_contents($file));

    $TTIMDB_CACHE[$torrentid] = $data;
...
//== iMDB ==//
$imdbData = tt_get_imdb_cache($row['id']);

$imdb = "";

$genre = ($imdbData && !empty($imdbData->Genre))
    ? "<span style='color:Silver;'>" . T_("GENRE") . ":</span> ".$imdbData->Genre
    : "<span style='color:Silver;'>" . T_("GENRE") . ":</span> . ";

$runtime = ($imdbData && !empty($imdbData->Runtime))
    ? "<span style='color:Silver;'>Runtime:</span> ".$imdbData->Runtime
    : "<span style='color:Silver;'>Runtime:</span> . . . ";

if ($imdbData && !empty($imdbData->imdbRating) && $imdbData->imdbRating != "N/A") {

    $ratingValue = floatval($imdbData->imdbRating);
    $stars = "";

    // === BROJ ZLATNIH ZVIJEZDA PO TVOJIM PRAVILIMA
    if ($ratingValue >= 8.5) {
        $gold = 5;
    } elseif ($ratingValue >= 7.0) {
        $gold = 4;
    } elseif ($ratingValue >= 5.5) {
        $gold = 3;
    } elseif ($ratingValue >= 4.0) {
        $gold = 2;
    } elseif ($ratingValue >= 2.0) {
        $gold = 1;
    } else {
        $gold = 0;
    }

    // === GENERISANJE 5 ZVIJEZDA
    for ($i = 1; $i <= 5; $i++) {
        if ($i <= $gold) {
            $stars .= "<i class='fa fa-star' style='color:#caac00; margin:0 1px;'></i>"; // zlatna
        } else {
            $stars .= "<i class='fa fa-star' style='color:#555; margin:0 1px;'></i>"; // siva
        }
    }

    // === FINALNI OUTPUT (zvijezde + broj)
    $rating = " " . $stars . " <span style='color:#999;'>(" . number_format($ratingValue, 1) . "/10)</span>";

} else {

    $rating = " <span style='color:#777;'>N/A</span> ";
	...
	foreach ($cols as $col) {
		switch ($col) {
			...
			case 'name':
				...
				print("<td class='ttable_col$x' nowrap='nowrap'>".(count($expandrows)?"<a href=\"javascript: klappe_torrent('t".$row['id']."')\"><img border=\"0\" src=\"".$site_config["SITEURL"]."/images/plus.gif\" id=\"pict".$row['id']."\" alt=\"Show/Hide\" class=\"showthecross\" /></a>":"")."&nbsp;<a title=\"".$row["name"]."\" href=\"torrents-details.php?id=$id&amp;hit=1\">$dispname</a><div style='margin-top:3px;font-size:10px;color:grey;padding-left:50px;'>  $genre | $runtime | <span style='color:Silver;'>Rating:</span> $stars </div></td>");
			break;
			case 'dl':
			...
			break;
			case 'comments':
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fa-regular fa-message' style='color: Orange; margin-right: 15px;'></i> <a href='comments.php?type=torrent&amp;id=$id'>" . number_format($row["comments"]) . "</a></button></td>\n");
			break;
			case 'nfo':
				if ($row["nfo"] == "yes")
			...
			break;
			case 'size':
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-database' style='color: Orange; margin-right: 15px;'></i> ".mksize($row["size"])." </button></td>\n");
			break;
			case 'completed':
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fa-solid fa-download' style='color: lime; margin-right: 15px;'></i> ".number_format($row["times_completed"])." </button></font></td>");
			break;
			case 'seeders':
				if ($row["seeders"] == "0")
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-up' style='color: Green; margin-right: 15px;'></i> ".number_format($row["seeders"])."</button></td>\n");
				else
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-up' style='color: Green; margin-right: 15px;'></i> ".number_format($row["seeders"])."</button></td>\n");
			break;
			case 'leechers':
				if ($row["leechers"] > "0")
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-down' style='color: Red; margin-right: 15px;'></i> " . number_format($row["leechers"]) . " </button></td>\n");
				else
				print("<td class='ttable_col$x' align='center'><button class='Tsiz'><i class='fas fa-angle-double-down' style='color: Red; margin-right: 15px;'></i> " . number_format($row["leechers"]) . " </button></td>\n");
			break;
			case 'health':
			...
			case 'external':
				if ($site_config["ALLOWEXTERNAL"]){
					if ($row["external"]=='yes')
						print("<td class='ttable_col$x' align='center'><i class='fa-solid fa-house-signal' style='color:red; font-size:16px; text-shadow: 0px 1px 0px #777; margin-right: 5px;'></i></td>\n");
					else
						print("<td class='ttable_col$x' align='center'><i class='fa-solid fa-house-chimney-window' style='color:green; font-size:16px; text-shadow: 0px 1px 0px #777; margin-right: 5px;'></i></td>\n");
				}
			break;
			case 'added':
...
function tt_hash_password(string $plain): string {
   $options = [
...
function tt_verify_password(string $plain, string $hash): bool {
   if (substr($hash, 0, 9) === '$argon2id') {
       return password_verify($plain, $hash);
   } elseif (strlen($hash) === 40 && preg_match('/^[0-9a-f]{40}$/i', $hash)) {
       // legacy SHA1
