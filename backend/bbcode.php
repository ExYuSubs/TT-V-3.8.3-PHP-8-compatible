	...
	if (start != end) {
        ...
        break;
			...
			if (start != end) {
				...
				var middle = "[video=" + body.substring(start, end) + "]";
				var rightpos = start + body.substring(start, end).length;
				var right = body.substring(rightpos, end);
				document.forms[form].elements[text].value = left + middle + right;
			} else {
				document.forms[form].elements[text].value = document.forms[form].elements[text].value + "[video=]";
			}
				break;
...
<table align='center' border='0' cellpadding='5' cellspacing='5' width='70%'>
  <tr class='b-title'>
    <th colspan="2" align='center' valign="middle"><table border="0" align="center" cellpadding="5" cellspacing="5">
        <tr>
          <td align="center" width="34"><input style="font-weight: bold;" class="button-bb" name="bold" value="B " onclick="javascript: BBTag('[b]','bold','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="34"><input style="font-style: italic;" class="button-bb" name="italic" value="I " onclick="javascript: BBTag('[i]','italic','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="34"><input style="text-decoration: underline;" class="button-bb" name="underline" value="U " onclick="javascript: BBTag('[u]','underline','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="50"><input class="button-bb" name="li" value="List " onclick="javascript: BBTag('[*]','li','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="34"><input class="button-bb" name="quote" value="QUOTE " onclick="javascript: BBTag('[quote]','quote','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="34"><input class="button-bb" name="url" value="URL " onclick="javascript: BBTag('[url]','url','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="34"><input class="button-bb" name="img" value="IMG " onclick="javascript: BBTag('[img]','img','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
          <td align="center" width="70"><input class="button-bb" name="video=" value="<?php echo T_("VIDEO"); ?>" onclick="javascript: BBTag('[video=',']','<?php echo $name; ?>','<?php echo $form; ?>')" /></td>
        </tr>
    </table>
    </th>  </tr>
  <tr class='b-row'>
    <td class='bb-comment' align='center' valign='top'><textarea name="<?php echo $name; ?>" rows="10" cols="90"><?php echo $content; ?></textarea></td>
