...
if ($owned)
	echo "[<a href='torrents-edit.php?id=" . (int)$row["id"] . "&amp;returnto=" . urlencode($_SERVER["REQUEST_URI"]) . "'><b>".T_("EDIT_TORRENT")."</b></a>]";
echo "</div>";
		...
		if ($scrape =='1'){
			...
			while ($trow = mysqli_fetch_assoc($tres)) {
				...
				if ($stats['seeds'] != -1) {
					...
					SQL_Query_exec("UPDATE `announce` SET `online` = 'yes', `seeders` = " . (int)$stats["seeds"] . ", `leechers` = " . (int)$stats["peers"] . ", `times_completed` = " . (int)$stats["downloaded"] . " WHERE `url` = " . sqlesc($ann) . " AND `torrent` = $id");
				} else {
...
if (!empty($row["image1"]) || !empty($row["image2"])) {
   ...
   $img2 = "";
  if ($row["image1"] != "")
    $img1 = "<img src='" . $site_config["SITEURL"] . "/uploads/images/" . rawurlencode($row["image1"]) . "' width='150' border='0' alt='' />";
  if ($row["image2"] != "")
    $img2 = "<img src='" . $site_config["SITEURL"] . "/uploads/images/" . rawurlencode($row["image2"]) . "' width='150' border='0' alt='' />";
	...
	$result = mysqli_num_rows($query);
		if($result == 0) {
			...
			while($row1 = mysqli_fetch_assoc($query))	{
                $arr = ["username" => "Unknown User", "privacy" => "strong"];
				...
				$percentcomp = ($row["size"] > 0) ? sprintf("%.2f", 100 * (1 - ($row1["to_go"] / $row["size"]))) : "0.00";    

				if ($site_config["MEMBERSONLY"]) {
					$res = SQL_Query_exec("SELECT id, username, privacy FROM users WHERE id=".$row1["userid"]."");
					$arr = mysqli_fetch_array($res);
                    
                    $arr["username"] = "<a href='account-details.php?id=" . (int)$arr["id"] . "'>" . htmlspecialchars($arr["username"], ENT_QUOTES, "UTF-8") . "</a>";
				}
				...
				if ($arr["privacy"] != "strong" || ($CURUSER["control_panel"] == "yes")) {
					print("<tr><td class='table_col2'>" . (int)$row1["port"] . "</td><td class='table_col1'>" . mksize($row1["uploaded"]) . "</td><td class='table_col2'>" . mksize($row1["downloaded"]) . "</td><td class='table_col1'>" . $ratio . "</td><td class='table_col2'>" . mksize($row1["to_go"]) . "</td><td class='table_col1'>" . $percentcomp . "%</td><td class='table_col2'>" . htmlspecialchars((string)$row1["seeder"], ENT_QUOTES, "UTF-8") . "</td><td class='table_col1'>" . htmlspecialchars((string)$row1["connectable"], ENT_QUOTES, "UTF-8") . "</td><td class='table_col2'>" . htmlspecialchars((string)$row1["client"], ENT_QUOTES, "UTF-8") . "</td><td class='table_col1'>" . $arr["username"] . "</td></tr>");
				}else{
					print("<tr><td class='table_col2'>" . (int)$row1["port"] . "</td><td class='table_col1'>" . mksize($row1["uploaded"]) . "</td><td class='table_col2'>" . mksize($row1["downloaded"]) . "</td><td class='table_col1'>" . $ratio . "</td><td class='table_col2'>" . mksize($row1["to_go"]) . "</td><td class='table_col1'>" . $percentcomp . "%</td><td class='table_col2'>" . htmlspecialchars((string)$row1["seeder"], ENT_QUOTES, "UTF-8") . "</td><td class='table_col1'>" . htmlspecialchars((string)$row1["connectable"], ENT_QUOTES, "UTF-8") . "</td><td class='table_col2'>" . htmlspecialchars((string)$row1["client"], ENT_QUOTES, "UTF-8") . "</td><td class='table_col1'>Private</td></tr>");
				}
...
function my_nfo_translate($nfo){
        ...
        for ($i = 0; $i < (strlen($nfo) - 1); $i++)
        {
                ...
                else
                {
                        ...
                        $last_was_ascii = strpos($all_chars, $char) !== false;
                }
        }
        return $tmp;
}
...
//DISPLAY NFO BLOCK
if($row["nfo"]== "yes"){
	$nfofilelocation = $nfo_dir . "/" . (int)$row["id"] . ".nfo";
	$filegetcontents = @file_get_contents($nfofilelocation);
    if ($filegetcontents === false) $filegetcontents = "";
...
begin_frame(T_("COMMENTS"));
	...
	if ($CURUSER) {
		echo "<center>";
		echo "<form name=\"comment\" method=\"post\" action=\"torrents-details.php?id=" . (int)$row["id"] . "&amp;takecomment=yes\">";
