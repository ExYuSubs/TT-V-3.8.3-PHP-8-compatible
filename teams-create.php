...
$sure = ($_GET['sure'] ?? '') === 'yes' ? 'yes' : '';
$del = $_GET['del'] ?? 0;
$team = htmlspecialchars($_GET['team'] ?? '', ENT_QUOTES, 'UTF-8');
$edited = (int)($_GET['edited'] ?? 0);
$id = (int)($_GET['id'] ?? 0);
...
$editmembers = $_GET['editmembers'] ?? 0;
$name = $_GET['name'] ?? '';
$image = $_GET['image'] ?? '';
$owner = $_GET['owner'] ?? '';
$info = $_GET['info'] ?? '';
